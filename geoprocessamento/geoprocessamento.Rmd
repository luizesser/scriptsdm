...
nome_shape_area_estudo <- here("geoprocessamento/input_data/shape_area_estudo/hydro_upper_prpy.shp")

# Nome do raster com SRTM
nome_raster_srtm <- here("geoprocessamento/input_data/raster_srtm/SRTM_AS_250[5880].tif")

# Nome da pasta que contém os rasters das variáveis no presente que serão reescalonadas.
pasta_rasters_presente <- here("geoprocessamento/input_data/rasters_bio_presente/")

# Nome da pasta que contém os rasters das variáveis nos cenários futuros que serão reescalonadas.
pasta_rasters_cenarios_futuros <- here("geoprocessamento/input_data/rasters_bio_futuro/")
```
...
# Nome do shapefile em que o SRTM reescalonado será gravado.
nome_shape_srtm <- here("geoprocessamento/output_data/shape_srtm_escalonado/srtm_10k_6933.shp")

# Nome do shapefile em que as variáveis do presente reescalonadas será gravado.
...
pasta_shape_futuro <- here("geoprocessamento/output_data/shape_bio_futuro_escalonadas")
...
# Caso não queira manter nenhuma, use `c()` 
nomes_variaveis_grid <-  c()#"hyriv_i", "next_do", "main_ri", "length_", "dist_dn", "dist_up", "catch_s", "upland_", "endorhe", "dis_av_", "ord_str", "ord_cla", "ord_flo", "hybas_l", "shp_lng")

# Nome da variável srtm do raster que será mantida durante o reescalonamento. 
# Estabelecer o nome correto -- no maximo 10 caracteres -- para as variáveis.
# Caso não queira manter nenhuma, use `c()` 
...
mapa_tmp <- shape_area_estudo %>% 
  map_of_area("Bacia Paraná-Paraguai")
...
shape_area_estudo <- shape_area_estudo %>% 
  reproject_shapefile(projecao_desejada)
...
mapa_tmp <- shape_area_estudo %>% 
  map_of_area("Bacia Paraná-Paraguai", crs_subtitle = F)
...
```{r geracao_shape_srtm, eval=T, include=T}
if (nome_shape_srtm %>% file_exists() == F){
  grid_srtm <- shape_grid_estudo %>% 
      add_raster(nome_raster_srtm, nome_var_srtm, largura_celula, altura_celula)
  ...
  grid_srtm <- nome_shape_srtm %>% readOGR(verbose = F)
}

grid_srtm %>%
  fortify_join() %>%
  map_of_area(fill="srtm")
```
...
if (nome_shape_presente %>% file_exists() == F) {
  ...
  grid_variaveis_presente <- shape_grid_estudo %>% 
    add_raster(arquivos_raster_presente, nomes_variaveis_presente, largura_celula, altura_celula)
