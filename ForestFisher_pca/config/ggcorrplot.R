...
my_ggcorrplot <- function(corr,
  ...
  # Choosing automatically inferior and superior limits of the legend title 
  limit_inf <- min(corr$value)
  limit_sup <- max(corr$value)
  if (!(limit_inf < -1 || limit_sup > 1)){
    limit_inf <- -1
    limit_sup <- 1
  }
  # adding colors
