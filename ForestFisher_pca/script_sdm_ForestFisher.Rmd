...
7. Install all other required packages (run commands using RStudio console)
     * `install.packages(c("raster", "rgdal", "sf", "rgeos", "ggplot2", "cowplot", "plotly", "mapview", "ggfortify", "scales", "factoextra ", "ggcorrplot", "prettyGraphs", "Rtsne", "DT", "parallel", "snow", "sdm", "FactoMineR", "rdist", "usdm", "dplyr", "tidyr", "purrr", "purrrlyr", "here", "fs", "stringr", "exactextractr", "gdalUtils", "dismo", "rgbif", "boot", "httr", "CoordinateCleaner"))`
...
# Future data
gcms <- c('cc', 'gg', 'mr', 'uk')
#WorldClim_data('future', variable = 'bioc', year = '2090', gcm = gcms, ssp = c('245','585'), resolution = 2.5) 
```
...
folder_current_rasters <- "~/projetos/ForestFisher/input_data/WorldClim_data_current"

# Directory name containing future rasters to be rescaled.
folder_future_rasters <- "~/projetos/ForestFisher/input_data/WorldClim_data_future"
```
...
output_shp_study_area <- "~/projetos/FF6/output_data/grid/Amazon_grid.shp"

# Name of R object (.rds) where the current rescaled variables will be saved.
output_shp_current <- "~/projetos/FF6/output_data/WorldClim_data_current_rescaled/Amazon_current.rds"

# Set scenarios names:
scenarios <- apply(expand.grid(gcms, c("ssp245","ssp585"),"2.5", c(2030,2050,2070,2090)), 1, paste, collapse="_")

# Name of R object (.rds) where the future rescaled variables will be saved.
output_shp_future <- here(paste0("~/projetos/FF6/output_data/WorldClim_data_future_rescaled/",
                                 scenarios,
                                 ".rds"))
```
...
# Following values build a grid with 5 x 5 km.
epsg = 6933
cell_width = 7000
cell_height = 7000
...
spp_data <- "~/projetos/FF4/scriptsdmmain/input_data/spp_data_complete.csv"
```
...
spp_output <- here("output_data/spp_pa_complete.gpkg")
```
...
spp_names_abv <- spp_names %>% 
    ...
    as.vector()
spp_names_abv <- spp_names_abv[4]
spp_names <- spp_names[4]
...
if (spp_output %>% file_exists() == F) {
  ...
  grid_matrix_pa %>% st_write(dsn = spp_output, driver = "GPKG")
} else {
...
```{r number of presences, eval=F, include=T}
...
var_predictors <-c("dim_1_bio","dim_2_bio","dim_3_bio","dim_4_bio","dim_5_bio","dim_6_bio","dim_7_bio")
```
...
```{r summ_pca_bio, eval=F, include=T}
pca_bio %>% 
  dt_pca_summ()
```
...
```{r contrib_pca_bio, eval=F, include=T}
pca_bio %>% 
  contrib_scree()
...
```{r contrib_dims_pca_bio, eval=F, include=T}
pca_bio %>% 
  contrib_dims()
```
...
```{r cos2_pca_bio, eval=F, include=T}
pca_bio %>% 
  pca_cos2()
```
...
```{r dims_cos2_pca_bio, eval=F, include=T}
pca_bio %>% 
  cos2_dims()
```


```{r cos2_corr_pca_bio, eval=F, warning=F, include=T}
pca_bio %>% 
  cos2_corr()
```


```{r pca_bio_biplot, eval=F, include=T}
pca_bio %>% 
...
```{r, eval=T}
df_potential_predictors <- df_potential_predictors %>%
  bind_cols(pca_bio$ind$coord[,1:n_var] %>% as.data.frame()) 
...
if (shp_preds %>% file_exists() == F){
  df_var_preditors <- df_potential_predictors %>% 
    select(var_predictors[1:n_var] %>% all_of())
...
df_pseudoabsences <- shp_matrix_pa %>%
  select(spp_names_abv) %>%
  pseudoabsences(df_var_preditors, spp_names_abv, method="envelope", folder_models) 
```
...
if(!file.exists(paste0('output_data/models/',spp_names_abv,'/',spp_names_abv,'.sdm'))){
  df_species %>%
...
ensembles <- gcm_ensemble(pred_means, 
                          ssp=c('current', 
                                'ssp245_2.5_2030', 'ssp245_2.5_2050', 'ssp245_2.5_2070', 'ssp245_2.5_2090', 
                                'ssp585_2.5_2030', 'ssp585_2.5_2050', 'ssp585_2.5_2070', 'ssp585_2.5_2090'))
...
```{r richness_map, eval=T, include=T}
ensemble_map(ensembles$current$current_pa_mean, grid_study_area, "Current", 'Presence')
```


## Ensembles for ForestFisher

```{r richness_map2, eval=T, include=T}
ensemble_map(ensembles$ssp245_2.5_2030$freq_wmean_auc, grid_study_area, "SSP2-4.5/2030", 'Frequence W-Mean AUC')
```

```{r richness_map2, eval=T, include=T}
ensemble_map(ensembles$ssp5$freq_wmean_tss, grid_study_area, "SSP2-4.5/2090", 'Frequence W-Mean TSS')
```

```{r richness_map2, eval=T, include=T}
ensemble_map(ensembles$ssp585$freq_wmean_tss, grid_study_area, "SSP5-8.5/2090", 'Frequence W-Mean TSS')
```
