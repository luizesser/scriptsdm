...
if (output_shp_current %>% file_exists() == F) {
  ...
  grid_current %>% as.data.frame %>% select(!'geometry') %>% write.csv(here("output_data/WorldClim_data_current_rescaled/Colossoma_current.csv"))
} else {
...
if (!all(output_shp_future %>% file_exists())) {
  for (i in 1:length(scenarios)) {
      ...
      dir <- grep('.rds', '.csv', output_shp_future[grep(scenarios[i], output_shp_future)])
      grid_future %>% as.data.frame %>% select(!'geometry') %>% write.csv(dir)  
  }
}
