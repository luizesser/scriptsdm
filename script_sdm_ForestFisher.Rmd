...
output_shp_study_area <- here("output_data/grid/Amazonia_grid.shp")

# Name of R object (.rds) where the current rescaled variables will be saved.
output_shp_current <- here("output_data/WorldClim_data_current_rescaled/Amazonia_current.rds")

# Set scenarios names:
scenarios <- apply(expand.grid(gcms, c("ssp585")), 1, paste, collapse="_")
...
names_var_shp_study_area <-  c("HYRIV_ID", "NEXT_DOWN", "MAIN_RIV", "LENGTH_KM", 
...
distribution_map(predictions_sp$clssm_mcrp$current_pa, grid_study_area)
```
      
      
## Frequence Consensus for Colossoma.macropomum in current scenario

```{r map_frequency, eval=T, include=T}
distribution_map(predictions_sp$clssm_mcrp$current_freq, grid_study_area)
```
...
ensemble_map(pred_means$current_freq_mean, grid_study_area)
```
...
ensemble_map(pred_means$current_pa_sums, grid_study_area)
```
