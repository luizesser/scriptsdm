...
# Future data
gcms <- c('cc', 'gg', 'mr', 'uk')
#WorldClim_data('future', variable = 'bioc', year = '2090', gcm = gcms, ssp = c('245'), resolution = 2.5) 
```
...
output_shp_study_area <- "~/projetos/ForestFisher/job/output_data/grid/Colossoma_grid.shp"

# Name of R object (.rds) where the current rescaled variables will be saved.
output_shp_current <- "~/projetos/ForestFisher/job/output_data/WorldClim_data_current_rescaled/Colossoma_current.rds"

# Set scenarios names:
scenarios <- apply(expand.grid(gcms, c("ssp245"),"2.5", 2090), 1, paste, collapse="_")

# Name of R object (.rds) where the future rescaled variables will be saved.
output_shp_future <- here(paste0("~/projetos/ForestFisher/job/output_data/WorldClim_data_future_rescaled/",
                                 scenarios,
                                 ".rds"))
```
...
if (output_shp_study_area %>% file_exists()== F){
  grid_study_area <- shape_study_area %>% 
      make_grid(cell_width, cell_height, names_var_shp_study_area, epsg=epsg) # epsg alvo
...
spp_data <- here('input_data/spp_data_complete.csv')
```
...
spp_output <- here("output_data/spp_pa_complete.shp")
```
...
spp_names <- c(#"Brycon.amazonicus", "Brycon.falcatus",
               #"Brycon.melanopterus", "Colossoma.macropomum",
               #"Hoplerythrinus.unitaeniatus", "Leporinus.brunneus",
               #"Leporinus.friderici", "Leporinus.klausewitzi",
               #"Lithodoras.dorsalis", "Megaleporinus.trifasciatus",
               #"Myloplus.asterias", "Myloplus.rubripinnis",
               #"Mylossoma.aureum", "Phractocephalus.hemioliopterus",
               #"Piaractus.brachypomus", "Platynematichthys.notatus",    
               "Pristobrycon.calmoni", "Pristobrycon.striolatus",    
               "Auchenipterichthys.thoracatus","Brycon.pesu"
               )
```
...
pred_means <- predictions_means(predictions_sp, scenarios)
...
## Frequence map in current scenario for all species

```{r map_frequency_ensemble, eval=T, include=T}
ensemble_map(ensembles$current$freq_mean, grid_study_area, "Current", 'Frequence')
```


## Richness map in current scenario

```{r richness_map, eval=T, include=T}
ensemble_map(ensembles$current$pa_sums, grid_study_area, "Current", 'Richness')
```
