...
```{r pred_distribution, eval=T, include=T}
...
```{r map_frequency_consensus, eval=T, include=T}
predictions_sp %>% 
  pluck(1) %>%
  distribution_map(grid_study_area)
```
...
```{r map_frequency_consensus_ssp245, eval=T, include=T}
...
```{r map_frequency_consensus_ssp585, eval=T, include=T}
...
```{r map_ensembles, eval=T, include=T}
...
```{r map_ensembles_585, eval=T, include=T}
predictions_sp$ssp585_freq %>%
  distribution_map_ensembles(grid_study_area)
```